...
EXTERN_C void* base64_decode(const char *data, size_t length, size_t *out_length)
{
	...
	while (i < length)
	{
		...
		while (i < length)
		{
			unsigned char decode = base64DecodeLookup[data[i++]];
			if (decode != xx)
			{
...
{
	const unsigned char *inputBuffer = (const unsigned char *)data;
...
#define CR_LF_SIZE 2
    ...
    * BASE64_UNIT_SIZE;
	...
	size_t i = 0;
	size_t j = 0;
	const size_t lineLength = separate_lines ? INPUT_LINE_LENGTH : length;
